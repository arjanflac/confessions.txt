...
export function parseMetadataLabel(label) {
  ...
  const unknownFields = Object.create(null);
  const warnings = [];
  const errors = [];

  if (new TextEncoder().encode(text).length > 16384) errors.push("Metadata label exceeds the 16 KiB limit.");
  if (!normalized) {
  ...
  for (let i = 0; i < parts.length; i += 1) {
    ...
    if (i === 0) {
      ...
      if ((hasPipeDelimitedLabel && !knownFirstField) || explicitTitle || freeTitle) {
        ...
        if (Object.hasOwn(fields, "TITLE")) errors.push("Duplicate TITLE field.");
  ...
  for (const aliases of [["ARTXID", "AR"], ["CSHA", "PROOF", "SHA", "HASH"], ["CID", "IPFS"]]) {
    if (aliases.filter(key => Object.hasOwn(fields, key)).length > 1) errors.push("Ambiguous alias fields: " + aliases.join("/"));
  }
  for (const [key, value] of Object.entries(fields)) {
    if (!value) errors.push(`Empty ${key} field.`);
  }
  ...
  if (steg) {
    warnings.push("STEG is public. Plaintext stays private only if the AGE password is strong and different from STEG.");
  }
...
export function generateLocalVerificationSteps(input) {
  ...
  if (artxid) {
    ...
    if (!ar.valid) errors.push(ar.message);
      ...
      if (publicSteg) {
        const stegValue = String(publicSteg);
        if (!stegValue || stegValue.includes("|") || stegValue.includes("\n") || hasControlChars(stegValue)) {
          ...
          commands.push(`# Extract encrypted payload with intentionally public STEG\n./confess extract --image locked_artifact.jpg --stego-pass=${shellQuote(stegValue)}`);
  ...
  if (csha) {
    ...
    if (!proof.valid) errors.push(proof.message);
      ...
      commands.push(`# Verify encrypted payload checksum\n./confess verify --file payload.age --csha ${proof.normalized}`);
    }
  } else if (artxid) {
  ...
  return {
    valid: errors.length === 0,
    commands: errors.length ? "" : commands.join("\n\n"),
    warnings,
    errors,
    notes: [
